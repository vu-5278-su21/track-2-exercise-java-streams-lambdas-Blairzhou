...
public class StreamUtils {
	...
	public static <T> Stream<List<T>> slidingWindow(List<T> data, int windowSize) {
		...
		if (windowSize < 1)
			return Stream.empty();
		return IntStream.range(0, data.size() - windowSize + 1).mapToObj(x -> data.subList(x, x + windowSize));
	}

	/**
	 * @ToDo:
	 *
	 *        This is a higher-order function that produces an averaging function
	 *        that operates on a List of objects.
	 *
	 *        The function takes a "property function" as a parameter. The property
	 *        function should return the property value associated with the object
	 *        that should contribute to the average. For example, you could provide
	 *        a function that extracts the heart rate data from a DataFrame.
	 *
	 *        The function returns a new function that computes the average of a
	 *        list of objects using the provided property function to extract a
	 *        property value for each object.
	 *
	...
	public static <T> Function<List<T>, Double> averageOfProperty(ToDoubleFunction<T> f) {
		return (List<T> window) -> {
			...
			return window.stream().map(x -> f.applyAsDouble(x)).mapToDouble(x->(double)x).average().getAsDouble();
		};
	}
